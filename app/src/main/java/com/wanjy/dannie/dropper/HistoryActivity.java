...
public class HistoryActivity extends AppCompatActivity {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()){
                    for(DataSnapshot history : dataSnapshot.getChildren()){
                        FetchDeliveryInformation(history.getKey());
                    }
                }
            }
            @Override
    ...
    private void FetchDeliveryInformation(String deliveryKey) {
        DatabaseReference historyDatabase = FirebaseDatabase.getInstance().getReference().child("history").child(deliveryKey);
        historyDatabase.addListenerForSingleValueEvent(new ValueEventListener() {
            @SuppressLint("SetTextI18n")
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()){
                    String deliveryId = dataSnapshot.getKey();
                    Long timestamp = 0L;
                    String distance = "";
                    ...
                    if(dataSnapshot.child("customerPaid").getValue() != null && dataSnapshot.child("courierPaidOut").getValue() == null){
                        if(dataSnapshot.child("distance").getValue() != null){
                            deliveryPrice = Double.valueOf(dataSnapshot.child("price").getValue().toString());
                            Balance += deliveryPrice;
    ...
    private void payoutRequest() {
        ...
        final Request request = new Request.Builder()
                .url("https://us-central1-dropper-23d67.cloudfunctions.net/payout")
                .post(body)
